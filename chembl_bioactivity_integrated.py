...
# ----------------------------
_UA = "chembl-bioactivity-report/0.2 (+https://github.com/Epineph/chembl-bioactivity-report)"
...
def pubchem_cid_from_name(name: str) -> int | None:
    ...
    # JSON first
    url_json = (
        f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{quote(nm)}/cids/JSON"
    )
    ...
    # TXT fallback
    url_txt = (
        f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{quote(nm)}/cids/TXT"
    )
                ...
                if "Information" in s:
                    for info in s["Information"]:
                        ...
                        if (
                            ...
                            val = " ".join(
                                v.get("String", "")
                                for v in info["Value"]["StringWithMarkup"]
                            )
...
def py3dmol_view_from_pubchem_cid(cid: int, width=420, height=320):
    ...
    try:
        ...
        if (
            ...
            or not r.ok
            or not r.text
            or ("V2000" not in r.text and "V3000" not in r.text)
        ):
            return None
...
def sort_dataframe(df: pd.DataFrame, by: str, ascending: bool) -> pd.DataFrame:
    ...
    if sn.notna().sum() >= max(1, len(s) // 2):
        ...
        df = df.sort_values(
            by="__sort_key__", ascending=ascending, na_position="last"
        ).drop(columns="__sort_key__")
    else:
        df = df.sort_values(
            by=by, ascending=ascending, key=lambda x: x.astype(str).str.casefold()
        )
...
def make_download_link(
    df: pd.DataFrame, filename: str, filetype: str = "csv", sep: str = ","
) -> str:
...
def make_blob_download_link(
    data: str | bytes, filename: str, mime: str, label: str | None = None
) -> str:
...
def safe_report_filename(compound: str, extension: str) -> str:
    slug = (
        re.sub(r"[^A-Za-z0-9._-]+", "_", compound or "compound").strip("_")
        or "compound"
    )
...
def display_report_downloads(compound: str, sections: list[dict], row_limit):
    ...
    display(
        HTML(
            make_blob_download_link(
                html_report,
                safe_report_filename(compound, "html"),
                "text/html",
    ...
    try:
        ...
        display(
            HTML(
                make_blob_download_link(
                    pdf_report,
                    safe_report_filename(compound, "pdf"),
                    "application/pdf",
                    "Download PDF report",
                )
            )
        ...
        display(
            Markdown(
                f"> PDF report unavailable: {e}. The HTML report can still be printed or saved as PDF from the browser."
            )
        )
...
def format_pk_simulation_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    columns = [
        ...
        "Elimination t1/2 (h)",
        "Absorption t1/2 (h)",
        "Apparent terminal t1/2 (h)",
        "Terminal phase driver",
    ...
    text_cols = {"Assumption", "Route", "Terminal phase driver"}
    numeric_cols = [col for col in out.columns if col not in text_cols]
...
def plot_pk_curves(
    curve_df: pd.DataFrame, mec_mg_l: float | None = None, mtc_mg_l: float | None = None
):
...
def formula_svg_html(label: str, formula: str) -> str:
    ...
    try:
        ...
        buf = io.StringIO()
        fig.savefig(
            buf, format="svg", bbox_inches="tight", transparent=True, pad_inches=0.08
        )
        plt.close(fig)
        svg = buf.getvalue()
...
def display_pk_formulas():
    ...
    blocks.extend(
        formula_svg_html(label, formula)
        for label, formula in pk_estimate_formula_items()
    )
    blocks.append(
    ...
    blocks.extend(
        formula_svg_html(label, formula) for label, formula in pk_formula_items()
    )
    blocks.append(
...
def interactive_mode():
    ...
    potency_threshold = widgets.FloatText(
        value=0.0,
        description="Max nM:",
        style={"description_width": "initial"},
    ...
    pk_controls = widgets.VBox(
        [
            ...
            widgets.HBox(
                [route_select, widgets.VBox([mec_input, mtc_input, injection_conc])]
            ),
    ...
    def on_click(_):
        with out:
            ...
            display(
                Markdown(
                    "Data sources: **ChEMBL** (bioactivity), **PubChem** (structure & properties), and transparent structure-based PK heuristics."
                )
            )
            report_sections = []
            ...
            try:
                ...
                if not df_pd.empty:
                    ...
                    report_sections.append(
                        {
                            "title": "Pharmacodynamic Bioactivities (Homo sapiens)",
                            "df": df_pd,
                        }
                    )
                    ...
                    display(
                        HTML(
                            make_download_link(
                                df_pd, "bioactivity.csv", "csv", sep=sep_choice.value
                            )
                        )
                    )
            ...
            if cid is None:
                display(
                    Markdown(
                        "> PubChem lookup failed; structure/properties unavailable."
                    )
                )
            ...
            if not df_basic.empty:
                ...
                report_sections.append(
                    {"title": "PubChem Basic Properties", "df": df_basic}
                )
                display(
                    HTML(
                        make_download_link(
                            df_basic, "pubchem_basic.csv", "csv", sep=sep_choice.value
                        )
                    )
                )
            ...
            descriptors = complete_pk_descriptors(descriptors, cid=cid, smiles=smiles)
            smiles = (
                smiles
            ...
            if not df_pk.empty:
                ...
                report_sections.append(
                    {
                        "title": "Predicted Pharmacokinetics (structure-based)",
                        "df": df_pk,
                        "note": PK_ESTIMATE_NOTE,
                    }
                )
                display(
                    HTML(
                        make_download_link(
                            df_pk,
                            "predicted_pharmacokinetics.csv",
                            "csv",
                ...
                display(
                    HTML(
                        make_download_link(
                            df_pk, "predicted_pharmacokinetics.xlsx", "xlsx"
                        )
                    )
                )
            ...
            if not descriptors:
                ...
                try:
                    ...
                    estimated_mec, estimated_mtc, threshold_df = (
                        estimate_exposure_thresholds(
                            pk_prediction,
                    ...
                    if not threshold_df.empty:
                        ...
                        for col in [
                            ...
                            if col in threshold_display.columns:
                                threshold_display[col] = pd.to_numeric(
                                    threshold_display[col], errors="coerce"
                                ).round(4)
                        ...
                        report_sections.append(
                            {
                                "title": "Estimated MEC/MTC From Reference Dose",
                                "df": threshold_display,
                            }
                        )
                    if mec_input.value and mec_input.value > 0:
                        display(
                            Markdown(
                                f"> Using manually entered MEC: **{mec:.4g} mg/L**."
                            )
                        )
                    elif estimated_mec is not None:
                        display(
                            Markdown(
                                f"> Estimated MEC from reference active dose: **{estimated_mec:.4g} mg/L**."
                            )
                        )
                    if mtc_input.value and mtc_input.value > 0:
                        display(
                            Markdown(
                                f"> Using manually entered MTC: **{mtc:.4g} mg/L**."
                            )
                        )
                    elif estimated_mtc is not None:
                        display(
                            Markdown(
                                f"> Estimated MTC from reference toxic dose: **{estimated_mtc:.4g} mg/L**."
                            )
                        )
                    ...
                    if fig is not None:
                        ...
                        display(
                            Markdown(
                                "> Matplotlib is unavailable; showing simulation tables only."
                            )
                        )
                    ...
                    if not sim_display.empty:
                        ...
                        report_sections.append(
                            {
                                "title": "Dose and Route Simulation Summary",
                                "df": sim_display,
                                "note": PK_SIMULATION_NOTE,
                            }
                        )
                        display(
                            HTML(
                    ...
                    if mec is None:
                        display(
                            Markdown(
                                "> Enter an MEC above 0 mg/L or a minimum active reference dose to calculate onset and duration above MEC."
                            )
                        )
                    display(
                        Markdown(
                            "> **Half-life note:** elimination half-life is calculated from CL/Vd and is route-independent in this linear one-compartment model. Apparent terminal half-life can become route-dependent when absorption is slower than elimination; route effects are also shown as absorption half-life and post-peak 50% decline time."
                        )
                    )
                    display(Markdown(f"> {PK_SIMULATION_NOTE}"))
                    display_pk_formulas()

                    metabolite_curve_df, metabolite_summary_df = (
                        simulate_active_metabolite_curves(
                            compound,
                            curve_df,
                            sim_summary,
                    ...
                    if not metabolite_curve_df.empty:
                        display(
                            Markdown("##### Active Metabolite / Active-Moiety Curve")
                        )
                        metabolite_fig = plot_active_metabolite_curves(
                            metabolite_curve_df
                        )
                                ...
                                report_sections.append(
                                    {
                                        ...
                                        "images": [
                                            {
                                                "title": "Active metabolite / active-moiety approximation",
                                                "data": metabolite_png,
                                                "mime": "image/png",
                                            }
                                        ],
                        ...
                        if not metabolite_summary_df.empty:
                            ...
                            for col in [
                                "Formation fraction",
                                "Metabolite half-life (h)",
                                "Relative potency vs parent",
                                "Peak active-moiety index",
                                "Peak time (h)",
                            ]:
                                metabolite_display[col] = pd.to_numeric(
                                    metabolite_display[col], errors="coerce"
                                ).round(4)
                            show(
                                metabolite_display,
                                classes="display compact cell-border",
                            ...
                            report_sections.append(
                                {
                                    "title": "Active Metabolite / Active-Moiety Summary",
                                    "df": metabolite_display,
                                }
                            )
                            display(
                                HTML(
            ...
            report_sections.append(
                {"title": "Active Metabolite / Prodrug Caveat", "df": df_metabolite}
            )
            ...
            if img is not None:
                ...
                if png:
                    ...
                    display(
                        Markdown(
                            "> Unable to render 2D structure (RDKit and PNG fallback both failed)."
                        )
                    )
            ...
            if show_3d.value:
                ...
                if viewer is not None:
                    ...
                    msg = (
                        "py3Dmol not available"
                        if not _P3D
                        else "No PubChem 3D conformer found or retrieval failed"
                    )
                    display(Markdown(f"> 3D viewer unavailable: {msg}."))
            else:
                display(
                    Markdown(
                        "> 3D structure skipped. Enable **Fetch 3D structure (slower)** to retrieve it."
                    )
                )
            ...
            if show_pubchem_props.value:
                ...
                if not df_exp.empty:
                    ...
                    report_sections.append(
                        {
                            "title": "Experimental / Computed Properties (PubChem)",
                            "df": df_exp,
                        }
                    )
                    display(
                    ...
                    display(
                        HTML(
                            make_download_link(
                                df_exp, "pubchem_properties.xlsx", "xlsx"
                            )
                        )
                    )
                else:
                    display(
                        Markdown(
                            "> No experimental/computed properties found (or parse failed)."
                        )
                    )
            else:
                display(
                    Markdown(
                        "> Full PubChem property scrape skipped. Enable **Fetch full PubChem properties (slower)** to retrieve it."
                    )
                )
