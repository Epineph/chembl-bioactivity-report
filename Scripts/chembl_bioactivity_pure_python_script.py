# ===========================
# Neat ChEMBL Bioactivity Report (Extended, robust + fixed)
# ===========================

import io, base64, json, re, requests, contextlib, warnings
...
# Default: 50 rows per page + offer common sizes incl. “All”
...
def pubchem_basic_props_df(cid: int) -> pd.DataFrame:
    ...
    Returns columns: Source, Property, Value
    """
    if _PCP:
        try:
            props = [
...
def pubchem_experimental_props_df(cid: int) -> pd.DataFrame:
    ...
    # Non-capturing groups (?:...) to avoid "match groups" warning
    patterns = [
        r'\bmelting point\b',
        r'\bboiling point\b',
    ...
    # Set na=False so NaNs don’t propagate warnings and behave as False
    ...
    # Broader fallback without capturing groups
    fallback_patterns = [
...
def py3dmol_view_from_pubchem_cid(cid: int, width=420, height=320):
    ...
    try:
        ...
        if not r.ok or not r.text or ("V2000" not in r.text and "V3000" not in r.text):
            return None
...
def interactive_mode():
    ...
    def on_click(_):
        with out:
            ...
            # Basic computed IDs/properties (restored function)
